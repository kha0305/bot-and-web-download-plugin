...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (ext === ".jar" || ext === ".zip") {
      ...
      cb(new Error("Chi chap nhan file .jar hoac .zip"), false);
    }
  },
  limits: {
...
const translationUpload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (ext === ".yml" || ext === ".yaml" || ext === ".txt") {
      ...
      cb(new Error("Chi ho tro file .yml, .yaml hoac .txt"), false);
    }
  },
  limits: {
...
app.post("/api/public/translate/text", async (req, res) => {
  try {
    ...
    const translated = await Translator.translateText(trimmed, engine, { skill });
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const translated = await Translator.translateContent(content, engine, {
        skill,
      });

      res.json({
        fileName: `VI_${originalName}`,
...
app.post(
  ...
  (req, res) => {
    try {
      ...
      req.files.forEach((f) => {
        const pluginName = f.originalname
          .replace(".jar", "")
          .replace(".zip", "");
        const plugin = {
...
app.use((err, req, res, next) => {
  ...
  if (
    ...
    (err.message.includes("Chi chap nhan file") ||
      err.message.includes("Chi ho tro file") ||
